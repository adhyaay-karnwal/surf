...
We believe the use of AI models in Breeze should be open.

We've built a UI to switch between models on every query. We have architected Breeze so that you can bring your own key for popular models or add your own custom model (either local or in the cloud).

Once added, your model will show up in the UI.

## Switching Models in Breeze

---

Breeze's interface allows you to easily switch between models on the fly and use the one that best fits your needs. Just select it in the switcher.
...
## Adding Your Own Model to Breeze

---

![byok](./assets/custom-model.webp)

If you have not yet setup your own AI model, follow the instructions for some [**common providers**](#common-providers) below. Once you have your model running, you can add it to Breeze by following these steps:

1. Open the Breeze settings through the app menu or using the shortcut `⌘/ctrl + ,`
...
Once you have added your model, you can start using it in Breeze. Select it from the dropdown at the top of the settings page or from the Chat or Smart Notes editor.

## Configuration Options

---

Here is a list of the configuration options you can set when adding a new model to Breeze:
...
To use models hosted by [OpenRouter](https://openrouter.ai/) first create an account, then generate an API key from their [settings page](https://openrouter.ai/settings/keys). Use the following configuration options to connect it to Breeze:
...
4. Add a new model in Breeze’s AI settings and configure it like below:
...
In theory any provider that offers a OpenAI compatible chat completions API should work with Breeze. In practice some providers are not fully compatible and can come with their own quirks. The only way to know for sure if a given provider works is by trying it out. Refer to the [Configuration Options](https://www.notion.so/Configuration-Options-199a5244a717800081a7ce12cef7bd93?pvs=21) above on what information is needed.

If you do get a provider working please let us know on [**Discord**](https://discord.gg/AKaBGfRXbw) or via [**E-Mail**](mailto:hello@breeze.breeze) and we will add it to this list so others can easily set it up as well. If you run into issues let us know as well and we’ll see if we can add support for your provider.

## Managed AI

---

You can download a version of Breeze with managed AI from the [Breeze website](https://breeze.breeze/).
