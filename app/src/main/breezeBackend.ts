import { isWindows } from '@breeze/utils'
import { spawn, type ChildProcess, execSync } from 'child_process'

import EventEmitter from 'events'
import { basename } from 'path'

export class BreezeBackendServerManager extends EventEmitter {
  ...
  start(): void {
    if (this.process) {
      this.emit('warn', 'breeze backend server is already running')
      return
    }
  ...
  stop(): void {
    if (!this.process) {
      this.emit('warn', 'breeze backend server is not running')
      return
    }
  ...
  private handleUnexpectedExit(): void {
    if (this.restartAttempts < this.maxRestartAttempts) {
      ...
      this.emit(
        'warn',
        `breeze backend server exited. restarting in ${this.restartDelay / 1000} seconds`
      )
      setTimeout(() => this.start(), this.restartDelay)
    } else {
      this.emit(
        'error',
        'max restart attempts reached. breeze backend server will not restart automatically'
      )
    }
  }
  ...
  private killExistingProcess(): void {
    try {
      ...
      this.emit('info', 'killed existing breeze backend server process')
    } catch (error) {
      this.emit('info', 'no existing breeze backend server process found')
    }
  }
}
