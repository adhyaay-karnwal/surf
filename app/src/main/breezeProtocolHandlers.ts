...
import { getResourceFileExtension, getResourceFileName, useLogScope } from '@breeze/utils'
import { SFFSMain, useSFFSMain } from './sffs'
import { SFFSRawResource, SFFSResource } from '@breeze/types'
...
const ALLOWED_HOSTNAMES = ['core', 'overlay', 'breeze']

const HOSTNAME_TO_ROOT = {
  core: '/Core/core.html',
  overlay: '/Overlay/overlay.html',
  breeze: '/Resource/resource.html'
}
...
export const handleBreezeFileRequest = async (req: GlobalRequest) => {
  try {
    const url = new URL(req.url)

    if (url.protocol !== 'breeze-internal:' && url.protocol !== 'breeze:') {
      log.error('Invalid protocol:', url.protocol)
      return new Response('Invalid Breeze protocol URL', { status: 400 })
    }

    if (!ALLOWED_HOSTNAMES.includes(url.hostname.toLowerCase())) {
      log.error('Invalid hostname:', url.hostname)
      return new Response('Invalid Breeze internal protocol hostname', { status: 400 })
    }
    ...
    if (targetPath === '/') {
      ...
      targetPath = rootPath
    } else if (url.hostname === 'breeze') {
      // Handle root requests (breeze://breeze/notebook/:id) and root assets
      ...
      if (match) {
        ...
        if (id) {
          ...
          if (url.pathname === `/${type}/${id}`) {
            const rootPath = HOSTNAME_TO_ROOT['breeze']
            if (!rootPath) {
              log.error('Invalid hostname for root path:', url.hostname)
              return new Response('Invalid Breeze internal protocol hostname', { status: 400 })
            }
            ...
            // For asset requests (breeze://breeze/notebook/:id/some/file.js), remove the type and ID prefix
            targetPath = url.href.replace(`breeze://breeze/${type}/${id}/`, '')
          }
        } else if (url.pathname === `/${type}`) {
          const rootPath = HOSTNAME_TO_ROOT['breeze']
          if (!rootPath) {
          ...
          // For root assets (breeze://breeze/notebook/assets/style.css)
    ...
    log.error('breeze internal protocol error:', err, req.url)
    return new Response('Internal Server Error', { status: 500 })
  }
}
...
const handleBreezeResourceDataRequest = async (req: GlobalRequest, resourceId: string) => {
  const sffs = useSFFSMain()
...
export const breezeInternalProtocolHandler = async (req: GlobalRequest) => {
  return handleBreezeFileRequest(req)
}

export const breezeProtocolHandler = async (req: GlobalRequest) => {
  try {
    const id = req.url.match(/^breeze:\/\/breeze\/resource\/([^\/\?]+)/)?.[1]
    if (id) {
      const searchParams = new URL(req.url).searchParams
      if (searchParams.has('raw') && searchParams.get('raw') !== 'false') {
        return handleBreezeResourceDataRequest(req, id)
      }
    }

    return handleBreezeFileRequest(req)
  } catch (err) {
    log.error('breeze protocol error:', err, req.url)
    return new Response('Internal Server Error', { status: 500 })
  }
}

export const breezeletProtocolHandler = async (req: GlobalRequest) => {
  try {
    const cspPolicy =
    ...
    // this is to not break existing breezelets that do not expect CSP
    if (isV2Protocol) {
    ...
    log.error('breezelet protocol error:', err, req.url)
    return new Response('Internal Server Error', { status: 500 })
  }
}

export const checkBreezeProtocolRequest = (url: string) => {
  try {
    const parsed = new URL(url)
    return (
      parsed.protocol === 'breeze:' && ALLOWED_HOSTNAMES.includes(parsed.hostname.toLowerCase())
    )
  } catch {
    return false
  }
