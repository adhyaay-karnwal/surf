import { type DragculaDragEvent } from '@breeze/dragcula'
import { type TabsService, type TabItem } from '@breeze/services/tabs'
import { DragTypeNames, type DragTypes } from '@breeze/types'
...
export function createTabsDragAndDrop(tabsService: TabsService) {
  const handleTabDrop = async (dragEvent: DragculaDragEvent<DragTypes>) => {
    if (!dragEvent.item?.data?.hasData(DragTypeNames.BREEZE_TAB)) {
      return
    }
    ...
    const draggedTab = dragEvent.item.data.getData(DragTypeNames.BREEZE_TAB) as TabItem
  ...
  const acceptTabDrag = (dragOperation: any) => {
    if (!dragOperation.item?.data?.hasData(DragTypeNames.BREEZE_TAB)) {
      return false
    }

    const draggedTab = dragOperation.item.data.getData(DragTypeNames.BREEZE_TAB) as TabItem
    const draggedTabId = draggedTab.id
