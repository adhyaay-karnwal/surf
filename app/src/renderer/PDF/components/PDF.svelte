<script lang="ts">
  ...
  import { WebViewEventReceiveNames, type WebViewReceiveEvents } from '@breeze/types'
  import { isDev, parsePDFViewerParams } from '@breeze/utils'
    ...
    if (!loading && !error) {
      if (pdfDownloadURL) {
        pdfSlick.loadDocument(pdfDownloadURL).then(async () => {
          ...
          if (!path.startsWith('breeze://breeze/resource')) {
