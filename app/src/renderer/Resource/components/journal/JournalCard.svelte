<script lang="ts">
  import { type Journal } from '@mist/services/journal'
  ...
  const colors = $derived(
    color ?? generateGradient(journalData?.folderName ?? journalData?.name ?? text ?? title, 220)
  )

  const handleClose = () => {
    if (journalData?.folderName) journalData.folderName = editorEl?.textContent
    if (journalData?.name) journalData.name = editorEl?.textContent
...
>
  {#if !editing}
    <span class="text">
      {`${journalData?.folderName ?? journalData?.name ?? title}`}
    </span>
  {:else}
    <span
      bind:this={editorEl}
