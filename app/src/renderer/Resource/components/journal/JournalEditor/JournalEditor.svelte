<script lang="ts">
  ...
  import { Button, JournalCover } from '@mist/ui'
  ...
  // Selectable colors for the journal
  // See journal.types.ts for explanation
  const colors: JournalCoverColor[] = [
    [
      ['#636161', '#636161'],
      ['#424141', '#424141'],
      ['#fff', '#fff']
  ...
  <div class="body">
    <section>
      ...
      <ColorPicker
        ...
        selected={journal.colorValue[0][0]}
        onpick={(c) => {
    ...
    <section>
      ...
      <p>You can add scribbles on the cover to make this journal your own.</p>
      {#if journal?.data?.customization?.coverScribble && journal?.data?.customization?.coverScribble.length > 0}
        <div class="ctrls">
          <button
            onclick={() => {
              journal.updateData({
