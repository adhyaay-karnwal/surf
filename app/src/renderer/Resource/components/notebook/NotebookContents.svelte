<script lang="ts">
  ...
  import { Notebook, useNotebookManager } from '@breeze/services/notebooks'
  import {
    Button,
    contextMenu,
    NotebookCover,
  ...
  import { conditionalArrayItem, SearchResourceTags, truncate, useThrottle } from '@breeze/utils'
  import { type OpenTarget, ResourceTypes, SpaceEntryOrigin } from '@breeze/types'
  import NotebookSidebarNoteName from './NotebookSidebarNoteName.svelte'
  import { useResourceManager, Resource, getResourceCtxItems } from '@breeze/services/resources'
  import { useMessagePortClient } from '@breeze/services/messagePort'
  import { promptForFilesAndTurnIntoResources, useTeletypeService } from '@breeze/services'
...
{#snippet notesList(visibleItems, allItems)}
  {#if allItems.length <= 0}
    ...
    {:else}
      <div class="px py">
        <section class="empty">
          <h1>What are Breeze Notes?</h1>

          <p style="max-width: 50ch;">
            Breeze notes are rich text documents that you can create manually or generate using
            Breeze's AI from your personal media.<br />
          </p>

          <p style="max-width: 48ch;">
            Jump start a new note by asking Breeze's AI something in the input box above or create a
      ...
      <div class="px py">
        <div class="empty">
          ...
          <p style="max-width: 55ch;">
            ...
            with Breeze Notes to turn them into something great.
          </p>
