<script lang="ts">
  import { Icon, IconConfirmation } from '@breeze/icons'
  import {
    copyToClipboard,
    generateID,
    isModKeyPressed,
  ...
  import { dataUrltoBlob } from '@breeze/utils/browser'
  ...
  import { DragTypeNames, type BookmarkTabState } from '@breeze/types'
  import type {
    DragTypes,
    ResourceTagsBuiltIn,
    SFFSResourceTag,
  ...
  $: if (tab && tab.title !== localTitle) {
    dispatch('set-breezelet-name', localTitle)
  }
  ...
  const changeResourceName = useDebounce(async (name: string) => {
    dispatch('set-breezelet-name', name)
  }, 500)
  ...
  const saveBreezeletAsNonSilent = async (spaceId?: string) => {
    dispatch('save-breezelet', { spaceId })
    saveState.set('saved')
  ...
  const renderHTMLPreview = async () => {
    ...
    const protocolVersion = $resource?.tags?.find(
      (tag) => tag.name === ResourceTagsBuiltInKeys.BREEZELET_PROTOCOL_VERSION
    )?.value
