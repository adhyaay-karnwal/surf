<script lang="ts">
  import { Journal, type JournalCoverColor } from "@mist/services/journals"
  import { clickOutside, generateID } from '@mist/utils'
  import { type Fn } from '@mist/types'
  import JournalCoverSticker from "./JournalCoverSticker.svelte"
  ...
  const scribbleValue = $derived(scribble ?? journal?.data?.customization?.coverScribble);

  const colorValue = $derived(journal?.colorValue ?? color ?? [
  ...
  let layers: {
    ...
    height: number
    color: string
  }[] = $state(journal?.data?.customization?.coverScribble ?? [])
  let undoStack: {
    path: string
    width: number
    height: number
  ...
  const oncomplete = (path: string) => {
    ...
    journal.updateData({
      customization: {
        ...journal.data.customization,
        coverScribble: layers
      }
    })
  }

  $effect(() => layers = journal?.data?.customization?.coverScribble ?? [])
      ...
      <span
        ...
        onkeydown={(e) => {
          if (e.key === 'Escape') {
            ...
            //oncancel?.()
            if (journal) {
              journal.updateData({ name: (e.target as HTMLSpanElement).textContent })
            }
          } else  if (e.key === 'Enter') {
            e.preventDefault()
            if (journal) {
        ...
        {@attach clickOutside(() => {
          if (journal && editorEl) {
            journal.updateData({ name: (editorEl as HTMLSpanElement).textContent })
          }
        })}
      >{journal?.nameValue ?? title}</span>
    {/if}
    <div class="left-band"></div>
    <div class="stickers" bind:this={stickersEl}>
      <!--<span class="sub-text">foobar</span>-->
      {#if (onpin || onunpin) && (isHovered || journal?.data?.pinned)}
        <div
          class="heart-sticker"
          class:pinned={journal?.data?.pinned}
          style:color={journal?.data?.pinned ? "var(--color-text-fallback)" : "var(--color-text-fallback)"}
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg"
            ...
            <path
              ...
              fill={journal?.data?.pinned ? "white" : "transparent"}
            />
          </svg>
        </div>
      {/if}
