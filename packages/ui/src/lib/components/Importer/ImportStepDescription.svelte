<script lang="ts">
  import type { BrowserTypeItem } from '@mist/types'
...
{#if currentStepIdx === 0}
  <p>
    Mist can import your browser history and bookmarks from your old browser so you can continue
    where you left off.
  </p>
  {#if initialImport}
    <p>Which browser did you use before Mist?</p>
  {/if}
{:else if currentStepIdx === 1}
  <p>Choose what data you want to import from {selectedBrowser?.name} into Mist.</p>
{:else if currentStepIdx === 2 && (importStatus === 'idle' || importStatus === 'importing')}
  <p>
    We are importing your data from {selectedBrowser?.name} into Mist. This may take a couple seconds.
  </p>
{:else if currentStepIdx === 2 && importStatus === 'done'}
  <p>Your data from {selectedBrowser?.name} has been imported to Mist successfully!</p>
