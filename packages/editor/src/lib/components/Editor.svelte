<script lang="ts">
  ...
  import { conditionalArrayItem, useAnimationFrameThrottle } from '@mist/utils'
  ...
  export let mistletComponent: ComponentType<SvelteComponent> | undefined = undefined
  ...
  const baseExtensions = createEditorExtensions({
    ...
    mistletComponent: mistletComponent,
      ...
      editorProps: {
        handleDOMEvents: {
          drop: (view, e) => {
            ...
            const tabId = e.dataTransfer?.getData(DragTypeNames.MIST_TAB_ID)
            if (tabId) {
              e.preventDefault()
              return false
            }
