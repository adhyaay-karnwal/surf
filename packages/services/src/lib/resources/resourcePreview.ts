...
import { ResourceJSON, ResourceNote, type Resource } from '@breeze/services/resources'
import {
...
export const getResourcePreview = async (resource: Resource, opts?: PreviewOptions) => {
  ...
  const userMediaResource = previewImageId ? `breeze://breeze/resource/${previewImageId}` : null
  ...
  try {
    if (resource instanceof ResourceJSON) {
      ...
      const image = userMediaResource ?? `breeze://breeze/resource/${resource.id}?raw`
      ...
      previewData = {
        ...
        url: canonicalUrl ?? parseStringIntoUrl(resource.metadata?.sourceURI ?? '')?.href ?? '',
        source: {
          text: hostname ? `Generated on ${hostname}` : 'Breezelet',
          imageUrl: undefined,
          icon: 'code-block'
        },
        theme: undefined
