...
export class ViewManager extends EventEmitterBase<ViewManagerEmitterEvents> {
  ...
  private async refillPool(type?: 'web' | 'breeze') {
    if (!type) {
      await Promise.all([this.refillPool('web'), this.refillPool('breeze')])

      return
    }

    this.log.debug('Refilling view pool', type)
    const viewPool = type === 'breeze' ? this.viewPoolBreezeProtocol : this.viewPoolWebPages
    ...
    for (let i = 0; i < needed; i++) {
      const fullData = {
        ...
        url: type === 'breeze' ? 'breeze://breeze/resource/blank' : 'about:blank',
        title: 'New Tab',
        faviconUrl: '',
  ...
  openResourceInSidebar(resourceId: string) {
    return this.openURLInSidebar(`breeze://breeze/resource/${resourceId}`)
  }
  ...
  private async prepareNewHomepage() {
    try {
      ...
      this.newHomepageView = await this.create({ url: 'breeze://breeze/notebook' }, true)
  ...
  async openNewHomepage() {
    try {
      if (!this.newHomepageView) {
        return this.openURLInSidebar('breeze://breeze/notebook')
      }
