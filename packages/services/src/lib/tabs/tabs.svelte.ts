...
export class TabsService extends EventEmitterBase<TabsServiceEmitterEvents> {
  ...
  private async prepareNewTabPage() {
    try {
      ...
      this.newTabView = await this.viewManager.create({ url: 'breeze://surf/notebook' }, true)
  ...
  async openNewTabPage() {
    try {
      if (!this.newTabView) {
        return this.create('breeze://surf/notebook')
      }
  ...
  async createResourceTab(resourceId: string, opts?: Partial<CreateTabOptions>) {
    ...
    const tab = await this.create(`breeze://surf/resource/${resourceId}`, opts)
    return tab
  }
