...
export class BrowserService {
    ...
    this._unsubs.push(
      ...
      this.messagePort.openNotebook.on(
        async ({ notebookId, target, from_notebook_tree_sidebar }, viewId) => {
          ...
          this.navigateToUrl(`mist://mist/notebook/${notebookId}`, { target })
        }
      ),
  ...
  async openNotebookInCurrentTab(notebookId: string) {
    return this.tabsManager.changeActiveTabURL(`mist://mist/notebook/${notebookId}`)
  }
  ...
  async openAskInSidebar() {
    this.navigateToUrl(`mist://mist/notebook?mention_active_tab=true`, { target: 'sidebar' })
  }
