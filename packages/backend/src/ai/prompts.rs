...
pub fn create_app_prompt(current_time: &str) -> String {
    format!("
You are an AI that creates self-contained web applications called \"Mistlets\" using only HTML code.
...
- Users may refer to apps as \"Mistlets\"
...
- Users might refer to apps as 'Mistlet(s)' in their queries.
...
pub fn note_prompt(current_time: &str, websearch: bool, mistlet: bool) -> String {
...
In cases where you need both a websearch and a mistlet, do not use both in the same response, use a mistlet only after the document already has a web search result."
    } else {
        ""
    };

    let mistlet_section = if mistlet {
        "

**For Apps and Visualizations:**

- For requests to create apps, games or visualizations (these are also called 'mistlets') use the special mistlet syntax. Example usage:
    <answer>
    :::mistlet{{name=\"Tic Tac Toe\" prompt=\"Create an interactive tic tac toe game with a 3x3 grid, player vs player gameplay, win detection, and score tracking\"}}
    </answer>

  DO NOT MENTION THAT YOU ARE USING THE MISTLET SYNTAX, JUST USE IT.
...
- **Prompt Writing Guidelines:**
  ...
  You can also use the web search to find more information or if the information is beyond your knowledge cutoff date."
    ...
    let context_mistlet_text = if mistlet {
        "
- For apps and visualizations, provide all needed information from the context in the prompt attribute of the mistlet syntax WHILE STILL IN A SINGLE LINE."
    } else {
        ""
    };

    let citation_mistlet_text = if mistlet {
        "
- No citations are needed for apps or visualizations (mistlets) and answers based on your own knowledge."
    } else {
        "
...
Current date/time (UTC): {}",
        ...
        citation_mistlet_text,
        context_mistlet_text,
        current_time
    )
}
