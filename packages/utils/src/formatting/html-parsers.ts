...
/**
 * Handle breezelet elements like:
 * ```html
 * <breezelet data-resource-id="edc810e3-9018-4b37-a13a-a30e492edf1d" data-prompt="Create an interactive demo of Apple" data-done="true" data-name="Liquid Glass UI Demo"></breezelet>
 * ```
 */
  ...
  try {
    ...
    const result = {
      ...
      children: [
        {
          type: 'link',
          url: `breeze://breeze/resource/${resourceId}`,
    ...
    console.error('Error processing breezelet:', error)
    // Fallback to original HTML if parsing fails
    const result = { type: 'html', value: toHtml(node) } satisfies MdastNodes
    state.patch(node, result)
    return result
